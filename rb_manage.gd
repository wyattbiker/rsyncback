...
const RSYNC_CONFIG_SECTION="rsyncback_config"
...
class ConfigArgs extends RsyncManage:
	...
	func _init(plugin_path: String, conf_resource: DefaultConfig):
	...
	func prepare_rsync_arguments(load_conf: bool)->ConfigArgs:	
		...
		if rsync_args.time_stamp_format=="":
			rsync_args.time_stamp_format="%04d-%02d-%02dT%02d%02d%02d"
		...
		if cd.find(rsync_args.source_path.path_join("")) == 0:
			res.code=-1
			res.description="Backup Destination Path %s cannot be within the Source Path %s project folder.\n Choose a Destination Path outside the project folder." %\
...
static func exec_rsync(cmgr:ConfigArgs)->Result:
	...
	if cmgr.dry_run_enabled.to_upper() == "Y":
		s="---- DRY RUN ---- DRY RUN ----\n"
	s=s+"Backup rsync log: "+cmgr.rsync_args.log_file_path+" @ "+log_time+\
			"\n\nCommand Used:\n"+cmgr.rsync_cmd_path+" "+sarguments+"\n\nRsync output: "+s+"\n\n"
			
	
	#sarguments=r""" 
	#-avi --mkpath --stats --delete   
	#--out-format="%M %'10'b %5f" --ignore-times   
	#--exclude-from="/home/user1/godot/plugin_test/addons/rsyncback/exclude.txt"   
	#--link-dest="/home/user1/myback2/plugin_test-rsync/2024-09-20T17_50_15"   
	#--log-file-format="%M %'10'b %5f"   
	#--log-file="/home/user1/myback2/plugin_test-rsync/logfiles/2024-09-21T12_41_39_log.txt"   
	#"/home/user1/godot/plugin_test/"   
	#"/home/user1/myback2/plugin_test-rsync/2024-09-21T12_41_39" """
	...
	var res:=Result.new()
	res=save_output_log(cmgr.rsync_args.log_file_path, cmgr.rsync_args.current_datetime+"_log.txt",s)
	if res.code != OK:
