...
var textedit_scene=preload("res://addons/rsyncback/scene/textedit.tscn")
...
func _ready_plugin():
	...
	if res.code != 0:
		...
		return
	print_debug("PLUGIN PATH:",plugin_path)
	cmgr=ConfigArgs.new(plugin_path, conf_resource)
	if verify_arguments():
		return false
...
func _on_run_backup_pressed():
	...
	print("Started: ",Time.get_unix_time_from_system())
	# Async execution but wait for result
	
	var res := await _exec_rsync(cmgr) 
	
	print("Ended: ",Time.get_unix_time_from_system())
...
func _exec_rsync(cmgr:ConfigArgs) -> Variant:	
	...
	if res.code == 0:
		display_log_blocking(res.value[0])		
	return res

func display_log_blocking(content: String):
	...
	#text_edit.popup_centered()
	var te_content:TextEdit=text_edit.get_node("%TextEditContent")
	#te_content.insert_text(content,0,0)
	var te_okbutton:Button=text_edit.get_node("%OKButton")
	te_okbutton.text="Cancel"
	te_okbutton.text="Close"
...
func refresh_ui():
	...
	$VBoxContainer/RsyncCmdPath/RsyncCmdPathClick.tooltip_text="Choose the RSync Executable. Minimum Version: " \
	...
	$VBoxContainer/DestPath/DestPathClick.tooltip_text="Choose a backup destination folder.\nBackups will be created in the folder: "+cmgr.rsync_args.project_name+" and a subfolder with the current datetime."
	...
	if cmgr.rsync_args.prev_backup != "":
		...
		$VBoxContainer/PrevBackup.text="No Previous Backup found yet at "+cmgr.rsync_args.dest_path.path_join(cmgr.rsync_args.project_name)
	...
	if cmgr.cfg_error_messages.is_empty():
		...
		$RSyncCommand.text="Error in Configuration. Click [b]View Messages[/b] button for more info."
...
func show_textedit(txt:="ABC", title:String="Text Edit", color:Color=Color.hex(messages_default_color)) -> Window:
	var textedit:Window=textedit_scene.instantiate() \
		.with_data(txt, title, color)
	var tscene=$"."
	var tpos=tscene.position+ tscene.global_position+(tscene.size - Vector2(textedit.size)) * 0.5
	textedit.position=tpos
	add_child(textedit)	
	return textedit
...
func set_config_view_tooltip() -> String:
	var s:="Click to view the configuration file: %s\n\
* Changes are saved when Run Rsync is clicked.\n* Deleting this file will create a new copy from %s\n\
* Open this file with Godot to edit as a resource if needed.
" % [conf_resource.resource_path, default_conf_resource.resource_path]
	return s
...
func confirm_edit_in_inspector():
	var s: String="""Manually Edit Configuration resource %s in Inspector. 
...
func set_exclude_file_view_tooltip() -> String:
	var s:="Click to view exclude file %s.\nList of file patterns that will be excluded from rsync backup.
Edit this file with any text editor. For rules visit [url=https://download.samba.org/pub/rsync/rsync.1#FILTER_RULES
]https://download.samba.org/pub/rsync/rsync.1[/url]
...
# Before saving this gets called to clear @tool/
# plugin test variables
func set_gui_vars():
	$RSyncBackupVesion.text=""
	$VBoxContainer/RsyncCmdPath.text=""
	$VBoxContainer/RsyncCmdPath/RsyncCmdPathClick.tooltip_text=""
	$VBoxContainer/SourcePath.text=""	
	$VBoxContainer/DestPath.text=""
	$VBoxContainer/DestPath/DestPathClick.tooltip_text=""
	$VBoxContainer/LogFilePath.add_theme_color_override("font_uneditable_color",Color.RED)
	$VBoxContainer/LogFilePath.text="No Log File Path yet."
	$VBoxContainer/ExcludeFilePath.text=""
	$VBoxContainer/ExcludeFilePath/ExcludeFilePathClick.tooltip_text=""

	$VBoxContainer/PrevBackup.text="No Previous Backup found yet"
	$VBoxContainer/PrevBackup.add_theme_color_override("font_uneditable_color",Color.RED)
		
	$VBoxContainer/ConfigFile.text = ""
	$VBoxContainer/ConfigFile/ConfigFileClick.tooltip_text=""
	$RSyncCommand.text="Error in Configuration. Click [b]View Messages[/b] button for more info."
	
	%RunBackup.disabled=true
	%ViewMessages.disabled=true
	%RunBackup/DryRun.tooltip_text=""
